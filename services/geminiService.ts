...
export const analyzeLessonFidelity = async (
    ...
    try {
        ...
        **[핵심 원칙 - 절대 준수]**:
        - 반드시 위에 제공된 '수업 녹취록'과 '단원 지도 계획' 텍스트만을 근거로 분석하세요.
        - 녹취록에 명시되지 않은 내용은 절대 추측하거나 일반적인 지식으로 보완하지 마세요.
        - 확인할 수 없는 내용은 "녹취록에서 확인되지 않음"으로 명시하세요.
        - AI의 배경 지식이나 일반적인 수업 상식으로 빈칸을 채우는 행위를 금지합니다.
        ...
        **[중요] 평가 원칙 - 반드시 준수**:
        ...
        - 교사의 어휘 선택이 부적절하거나 용어 오개념이 있으면 suggestions는 5점 이하
        - 학생들과의 상호작용이 형식적이면 suggestions는 5점 이하
        ...
        1. achievementAlignment (교육과정 성취기준 정합성): 점수(10점 만점)와 상세 피드백
           - 피드백에는 반드시 녹취록에서 발견된 구체적인 발화 내용을 인용하세요.
           ${!plan ? '- 성취기준이 없으므로 일반적인 교육목표 달성도를 평가해주세요.' : ''}
        2. contentAccuracy (내용 정확성 및 오개념 분석): 점수(10점 만점)와 상세 피드백
           - 녹취록에 실제로 등장한 내용만 평가하고, 없는 내용은 언급하지 마세요.
        3. suggestions (추가 및 제언): 점수(10점 만점)와 상세 피드백
           - 다음 내용을 반드시 포함하여 분석하세요:
             * 교사가 사용한 단어의 적절성 및 용어/개념의 정확도
             * 학생들과의 실질적인 상호작용 질(질문의 깊이 등)
             * 수업에서 **추가**하거나 **제거**해야 할 요소, 또는 더 **발전**적인 제안
           - "단순히 잘했다"는 칭찬보다는 실제 수업 발화를 근거로 한 개선점을 구체적으로 제언하세요.
...
export const generateFinalReport = async (studentName: string, history: any[]): Promise<string> => {
    try {
      ...
      **[핵심 원칙 - 절대 준수]**:
      ...
      반드시 기록에서 확인된 구체적인 사례를 인용하여 작성할 것.
      모든 응답은 한글로 작성.
    `;
...
export const generateStudentReport = async (studentName: string, observationData: any): Promise<string> => {
    try {
      ...
      **[핵심 원칙 - 절대 준수]**:
      - 반드시 위에 제공된 '관찰 내용'과 '성장 포인트'에 기록된 사실만을 근거로 작성하세요.
      - 관찰 기록에 없는 내용(예: 학생의 성격, 태도, 미래 가능성 등)을 추측하지 마세요.
      - "~할 것으로 기대됩니다" 같은 추측 표현 금지.
      - 모든 응답은 한글로 작성.
...
**[핵심 원칙 - 절대 준수]**:
- 반드시 위의 '수업 녹취록' 텍스트에 실제로 등장하는 내용만 추출하세요.
- 녹취록에 명시되지 않은 학생 활동은 절대 추측하거나 추가하지 마세요.
- 학생 이름이 녹취록에 직접 언급된 경우에만 포함하세요.
- 녹취록에 없는 내용을 상상하거나 일반적인 수업 패턴으로 보완 금지.
...
**주의사항**:
- 명단에 있는 학생 이름만 추출
- 실제로 녹취록에 언급된 학생만 포함 (없으면 빈 배열 반환)
